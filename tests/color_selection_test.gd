...
func _run() -> void:
	for color in range(4):
		...
		for color_name in ["Green", "Yellow", "Blue", "Red"]:
			var place: Sprite2D = board.get_node("Sprite2D_Board/BaseArtwork" + color_name)
		...
		assert(card.position == Vector2(1510, 138))
		assert(card.get_child(0).portrait.texture.resource_path.ends_with("player 4.jpg"))
		# Entry and subsequent movement still use the chosen color's path.
		# Avoid a random movement item changing the expected one-square move.
		group.Pieces[0].EquipItem(0)
		group.Pieces[0].EquipItem(1)
