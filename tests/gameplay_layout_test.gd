...
func _ready() -> void:
	...
	for resolution in [Vector2i(1920, 1080), Vector2i(2400, 1080), Vector2i(2520, 1080), Vector2i(1920, 1440)]:
		...
		assert(bottom_right.y <= resolution.y - 9.0)
		var dice: Node2D = game.get_node("CoreGamplay/Dice/DiceRoot")
		assert(dice.global_position.x + 190.0 < top_left.x)
		assert(dice.global_position.x - 180.0 >= 39.0)
		assert(dice.global_scale.is_equal_approx(Vector2.ONE * 0.62))
	...
	assert(hud.hero_name.text == "Pirunrun - (Mage)")
	...
	assert(is_equal_approx(hud.cards[int(other.CurrentPlayerColor)].position.y, 138.0))
	...
	assert(hud.chat_bubbles[int(GameManager.LocalPlayerColor)].visible)
	for title in ["Battle Status", "Knowledge", "Domain Authority"]:
	...
	hud._refresh_recommendations()
	assert(hud.recommendations.size() == 4)
	print("PASS: two-round inspection, animated ranking, chat, skill popups and four fixed hero portraits")
	get_tree().quit()
